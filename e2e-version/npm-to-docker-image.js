    ...
    const makeRequest = () => {
      ...
      const req = https.get(url, { timeout: 10000 }, (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            console.warn(`Server error ${res.statusCode} on attempt ${attempts}/${maxRetries} for ${url}. Retrying in ${retryDelay * attempts}ms...`);
            timeoutId = setTimeout(makeRequest, retryDelay * attempts); // Exponential backoff
          } else {
      ...
      const handleRequestError = (err) => {
        if (attempts < maxRetries && isRetryableNetworkError(err)) {
          console.warn(`Network error on attempt ${attempts}/${maxRetries} for ${url}: ${err.message}. Retrying in ${retryDelay * attempts}ms...`);
          timeoutId = setTimeout(makeRequest, retryDelay * attempts); // Exponential backoff
        } else {
          reject(err);
        }
